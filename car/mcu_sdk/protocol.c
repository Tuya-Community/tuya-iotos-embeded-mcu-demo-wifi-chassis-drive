...
{
  {DPID_FORWARD, DP_TYPE_BOOL},
  {DPID_BACKWARD, DP_TYPE_BOOL},
  {DPID_SPEED, DP_TYPE_VALUE},
 ...
 */
static uint8_t f_forward=0,f_backward=0, f_stop=1,f_speed=0;
void all_data_update(void)
{
    //#error "请在此处理可下发可上报数据及只上报数据示例,处理完成后删除该行"
   
    //此代码为平台自动生成，请按照实际数据修改每个可下发可上报函数和只上报函数
    mcu_dp_bool_update(DPID_FORWARD,f_forward); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_BACKWARD,f_backward); //BOOL型数据上报;
    mcu_dp_value_update(DPID_SPEED,f_speed); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_STOP,f_stop); //BOOL型数据上报;
...
******************************************************************************/
/*****************************************************************************
函数名称 : dp_download_forward_handle
功能描述 : 针对DPID_FORWARD的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_forward_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    Move_Stop;
    f_forward = mcu_get_dp_download_bool(value,length);
    if(f_forward == 0) {
        //开关关
			f_stop=1;
    }else {
		...
		mcu_dp_bool_update(DPID_STOP,f_stop);
		f_backward=0;
		mcu_dp_bool_update(DPID_BACKWARD,f_backward);
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_FORWARD,f_forward);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_backward_handle
功能描述 : 针对DPID_BACKWARD的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_backward_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    Move_Stop;
    f_backward = mcu_get_dp_download_bool(value,length);
    if(f_backward == 0) {
        //开关关
			f_stop=1;
    }else {
		...
		mcu_dp_bool_update(DPID_STOP,f_stop);
		f_forward=0;
		mcu_dp_bool_update(DPID_FORWARD,f_forward);
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_BACKWARD,f_backward);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
static unsigned char dp_download_speed_handle(const unsigned char value[], unsigned short length)
{
    ...
    f_speed = mcu_get_dp_download_value(value,length);
    /*
    //VALUE类型数据处理
    
    */
    TIM16_PWM_Set(f_speed);
    //处理完DP数据后应有反馈
    ret = mcu_dp_value_update(DPID_SPEED,f_speed);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
//    }
  mcu_dp_bool_update(DPID_FORWARD,f_forward);
	mcu_dp_bool_update(DPID_BACKWARD,f_backward);
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_STOP,f_stop);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        case DPID_FORWARD:
            //前进处理函数
            ret = dp_download_forward_handle(value,length);
        break;
        case DPID_BACKWARD:
            //后退处理函数
            ret = dp_download_backward_handle(value,length);
        break;
        case DPID_SPEED:
            //速度处理函数
